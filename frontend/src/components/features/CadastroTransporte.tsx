...
const CadastroTransporte: React.FC = () => {
  ...
  useEffect(() => {
    ...
    apiService
      .listarLotes()
      .then((l) => { if (ativo) setLotes(l); })
      .catch((e: Error) => { if (ativo) setErro(e.message); })
      .finally(() => { if (ativo) setIsLoadingLotes(false); });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErro(err instanceof Error ? err.message : 'Não foi possível registrar o transporte.');
    } finally {
  ...
  return (
    ...
    >
      {isLoadingLotes ? (
        <StatusBanner variant="loading">Carregando lotes disponíveis...</StatusBanner>
      ) : lotes.length === 0 ? (
        <p style={styles.emptyState}>
          Nenhum lote foi criado ainda. Cadastre um lote primeiro na aba &quot;Cadastrar Lote&quot;.
        </p>
      ) : (
        <form onSubmit={handleSubmit} style={styles.formGrid}>
          {/* Passo 1 — lote */}
          <div style={styles.field}>
            <label style={styles.label} htmlFor="lote-transporte">1. Lote</label>
            <select
              id="lote-transporte"
              style={styles.select}
              value={loteId}
          ...
          <div className="ecotrack-field-row" style={styles.fieldRow}>
            <div style={styles.field}>
              <label style={styles.label} htmlFor="cnpj-remetente">3. CNPJ do remetente</label>
              <input
                id="cnpj-remetente"
                style={styles.input}
            ...
            <div style={styles.field}>
              <label style={styles.label} htmlFor="cnpj-destinatario">4. CNPJ do destinatário</label>
              <input
                id="cnpj-destinatario"
                style={styles.input}
                placeholder="00.000.000/0000-00"
                value={cnpjDestinatario}
                onChange={(e) => setCnpjDestinatario(formatarCNPJ(e.target.value))}
              />
            </div>
          </div>
